...
def student_view_attendance(request):
    student = get_object_or_404(Students, admin=request.user.id)  # Safely get the student
    course = student.course_id  # Get the student's course
    subjects = Subjects.objects.filter(course_id=course)  # Get subjects for the course
...
def student_view_attendance_post(request):
    ...
    # Get the subject object, safely handle non-existent subjects
    subject_obj = get_object_or_404(Subjects, id=subject_id)

    # Get the logged-in user's student object
    stud_obj = get_object_or_404(Students, admin=request.user)
    ...
    # Render the attendance data in the specified template
    return render(request, 'student_template/student_attendance_data.html', context)
